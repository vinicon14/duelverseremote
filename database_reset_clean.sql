-- ============================================================================
-- RESET COMPLETO - EXCLUSÃO DE TODOS OS DADOS E ESTRUTURAS
-- ============================================================================
-- ATENÇÃO: Este script irá DELETAR TODOS OS DADOS da plataforma
-- Execute com cuidado!
-- ============================================================================
...
BEGIN
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public') LOOP
        EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(r.tablename || '_policy') || ' ON public.' || quote_ident(r.tablename) || ' CASCADE';
...
-- Usuários veem solicitações enviadas ou recebidas
...
-- Usuários podem atualizar solicitações recebidas
...
-- Usuários podem se juntar a duelos
CREATE POLICY "Users join duels" 
ON public.players FOR INSERT 
TO authenticated 
WITH CHECK (auth.uid() = user_id);

-- Jogadores podem atualizar seu próprio status
...
-- Sistema pode inserir histórico (via trigger ou function)
CREATE POLICY "System insert match history" 
ON public.match_history FOR INSERT 
TO authenticated 
WITH CHECK (true);

-- Admins podem deletar histórico
...
-- Usuários podem deletar suas mensagens
...
-- Todos podem ver torneios
CREATE POLICY "All view tournaments" 
ON public.tournaments FOR SELECT 
TO authenticated 
USING (true);

-- Admins podem gerenciar torneios
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM public.profiles) = 1 THEN
    ...
    VALUES (NEW.id, 'admin');
  ELSE
    -- Usuários normais recebem role 'user'
    INSERT INTO public.user_roles (user_id, role)
    VALUES (NEW.id, 'user');
  END IF;
...
-- 2. Todos os usuários antigos serão removidos
-- 3. Crie uma nova conta (será automaticamente admin)
-- 4. A plataforma estará pronta para uso!
-- ============================================================================
