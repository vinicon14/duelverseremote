...
const refreshOnlineCounter = async () => {
  try {
    await fetch(
      `${import.meta.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/discord-presence-counter`,
      {
        method: "POST",
        keepalive: true,
        headers: {
...
export const useDiscordPresence = (userId: string | undefined) => {
  useEffect(() => {
    ...
    const handleUnload = () => {
      ...
      try {
        const url = `${import.meta.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/discord-presence`;
        const blob = new Blob(
      ...
      try {
        const counterUrl = `${import.meta.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/discord-presence-counter`;
