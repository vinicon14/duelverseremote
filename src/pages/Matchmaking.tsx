              ...
              {matchFound && (
                <div className="space-y-6">
                  <div className="text-center py-6 sm:py-8">
                    ...
                    <h3 className="text-xl sm:text-2xl font-bold mb-2 text-green-500">Oponente Encontrado!</h3>
                    <p className="text-lg text-foreground font-semibold mb-2">vs {matchFound.opponentName}</p>
                    <p className="text-sm text-muted-foreground">Clique no botão abaixo para entrar na chamada de vídeo</p>
                  </div>
                  <Button onClick={joinDuel} className="w-full btn-mystic" size="lg">
                    <Video className="mr-2 h-5 w-5" /> Entrar na Chamada de Vídeo
                  </Button>
                  <Button onClick={cancelSearch} variant="outline" className="w-full">Cancelar</Button>
                </div>
              )}
              ...
              {!searching && !matchFound && (
                <div className="space-y-4">
                  <div className="text-center py-6 sm:py-8">
                    ...
                    <p className="text-sm sm:text-base text-muted-foreground mb-6">Vamos encontrar um oponente para você</p>
                  </div>
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <p className="text-xs text-muted-foreground text-center">
                      {isRanked ? "✅ Vale pontos no ranking" : "❌ Não vale pontos no ranking"}
                    </p>
                  </div>
                  ...
                  {isMagic && (
                    <div className="space-y-3 mb-4">
                      <label className="text-sm font-medium">Número de Jogadores</label>
                      <div className="grid grid-cols-2 gap-2">
                        <Button type="button" variant={playerCount === 2 ? "default" : "outline"} onClick={() => setPlayerCount(2)} className={playerCount === 2 ? "btn-mystic text-white" : ""}>
                          <Users className="w-4 h-4 mr-1" /> 2 Jogadores
                        </Button>
                        <Button type="button" variant={playerCount === 4 ? "default" : "outline"} onClick={() => setPlayerCount(4)} className={playerCount === 4 ? "btn-mystic text-white" : ""}>
                          <Users className="w-4 h-4 mr-1" /> 4 Jogadores
                        </Button>
                      </div>
                      <p className="text-xs text-muted-foreground text-center">
                        {playerCount === 4 ? "🎭 Free-for-All com 4 jogadores" : "⚔️ Duelo 1v1"}
                      </p>
                    </div>
                  )}
                  ...
                  <Button onClick={joinQueue} className="w-full btn-mystic" size="lg">
                    ...
                    Buscar Partida {isRanked ? 'Ranqueada' : 'Casual'} {isMagic && playerCount === 4 ? '(4P)' : ''}
                  </Button>
                </div>
              )}
              ...
              {searching && !matchFound && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-center text-muted-foreground">{remainingTime}s restantes antes do timeout</p>
                  </div>
                  <Button onClick={cancelSearch} variant="outline" className="w-full">Sair do Lobby</Button>
                </div>
              )}
            </div>
          </Card>
