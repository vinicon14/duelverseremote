...
export default function BuyDuelCoins() {
  ...
  const handleBuyPix = async (pkg: DuelCoinsPackage) => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        toast({ title: t('buyCoins.pixError'), description: data?.error, variant: "destructive" });
      }
    } catch (error: any) {
      console.error('Error creating PIX:', error);
      toast({ title: t('buyCoins.paymentError'), description: error.message, variant: "destructive" });
    } finally {
      setBuying(null);
    }
  };
  ...
  const handleCopyPix = async () => {
    ...
    try {
      ...
      setCopied(true);
      toast({ title: t('buyCoins.pixCopied') });
      setTimeout(() => setCopied(false), 3000);
    } catch {
      toast({ title: t('buyCoins.pixCopyError'), variant: "destructive" });
    }
  };

  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'paid': return <Badge className="bg-green-500">{t('buyCoins.statusPaid')}</Badge>;
      case 'pending': return <Badge variant="outline">{t('buyCoins.statusPending')}</Badge>;
      case 'cancelled': return <Badge variant="destructive">{t('buyCoins.statusCanceled')}</Badge>;
          ...
          {packages.length === 0 ? (
            <Card className="card-mystic">
              <CardContent className="py-12 text-center">
                ...
                <h3 className="text-xl font-semibold mb-2">{t('buyCoins.noPackages')}</h3>
                <p className="text-muted-foreground">{t('buyCoins.noPackagesDesc')}</p>
              </CardContent>
            </Card>
          ) : (
                  ...
                  {pkg.is_featured && (
                    <div className="absolute top-2 right-2">
                      <Badge className="bg-primary">
                        <Star className="w-3 h-3 mr-1" /> {t('buyCoins.popular')}
                      </Badge>
                    </div>
                  )}
                  {pkg.image_url && (
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        ...
                        {buyingCard === pkg.id ? t('buyCoins.redirecting') : t('buyCoins.payCard')}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
          ...
          {orders.length > 0 && (
            <Card className="card-mystic">
              <CardHeader>
                ...
                <CardDescription>{t('buyCoins.ordersDesc')}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="rounded-md border">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>{t('buyCoins.colDate')}</TableHead>
                        <TableHead>{t('buyCoins.colCoins')}</TableHead>
                        <TableHead>{t('buyCoins.colValue')}</TableHead>
                        <TableHead>{t('buyCoins.colStatus')}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {orders.map((order) => (
          ...
          {pixData && (
            <div className="space-y-4 text-center">
              <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                <p className="text-sm text-muted-foreground">{t('buyCoins.pixAmount')}</p>
              ...
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">{t('buyCoins.pixCopyLabel')}</p>
              ...
              <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                ...
                {t('buyCoins.pixWaiting')}
              </div>

              <p className="text-xs text-muted-foreground">
                {t('buyCoins.pixExpiresNote')}
              </p>
            </div>
          )}
        </DialogContent>
