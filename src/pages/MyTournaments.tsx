...
const MyTournaments = () => {
  ...
  const fetchOpponentsForTournaments = async (tournamentIds: string[], _userId: string) => {
  ...
  const challengeOpponent = async (opponentId: string, tournamentId: string) => {
    ...
    try {
      ...
      if (existingDuels && existingDuels.length > 0) {
        toast({
          title: t('myTournaments.alreadyDueling'),
          description: t('myTournaments.alreadyDuelingDesc'),
          variant: "destructive",
        });
      ...
      if (opponentDuels && opponentDuels.length > 0) {
        toast({
          title: t('myTournaments.opponentBusy'),
          description: t('myTournaments.opponentBusyDesc'),
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: t('myTournaments.challengeSent'),
        description: t('myTournaments.challengeSentDesc'),
      });

      navigate(`/duel/${duelData.id}`);
    } catch (error: any) {
      toast({
        title: t('myTournaments.errorChallenge'),
        description: error.message,
        variant: "destructive",
      });
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'upcoming':
        return <Badge className="bg-blue-500/20 text-blue-400">{t('myTournaments.statusUpcoming')}</Badge>;
      case 'active':
        return <Badge className="bg-green-500/20 text-green-400">{t('myTournaments.statusActive')}</Badge>;
      case 'completed':
        return <Badge className="bg-gray-500/20 text-gray-400">{t('myTournaments.statusCompleted')}</Badge>;
      case 'expired':
        return <Badge className="bg-red-500/20 text-red-400">{t('myTournaments.statusExpired')}</Badge>;
      default:
        ...
        <CardContent className="space-y-4">
          {tournament.current_round && tournament.status === 'active' && (
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>{t('myTournaments.round', { number: tournament.current_round })}</span>
            </div>
          )}

          {showChallenge && tournamentOpponents.length > 0 && (
            <div className="space-y-2">
              <p className="text-sm font-medium text-muted-foreground">{t('myTournaments.yourOpponents')}</p>
        ...
        <Tabs defaultValue="active" className="space-y-6">
          <TabsList className="grid w-full max-w-md grid-cols-3">
            <TabsTrigger value="upcoming">
              {t('myTournaments.tabUpcoming', { count: upcomingTournaments.length })}
            </TabsTrigger>
            <TabsTrigger value="active">
              {t('myTournaments.tabActive', { count: activeTournaments.length })}
            </TabsTrigger>
            <TabsTrigger value="completed">
              {t('myTournaments.tabCompleted', { count: completedTournaments.length })}
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="upcoming">
            {upcomingTournaments.length === 0 ? (
              <Card className="card-mystic text-center py-12">
                ...
                <h3 className="text-xl font-semibold mb-2">{t('myTournaments.noUpcoming')}</h3>
                <p className="text-muted-foreground mb-4">
                  {t('myTournaments.noUpcomingDesc')}
                </p>
                <Button
                  onClick={() => navigate('/tournaments')}
                  className="btn-mystic text-white"
                >
                  {t('myTournaments.viewAvailable')}
                </Button>
              </Card>
            ) : (
          ...
          <TabsContent value="active">
            {activeTournaments.length === 0 ? (
              <Card className="card-mystic text-center py-12">
                <Trophy className="w-16 h-16 mx-auto text-primary/50 mb-4" />
                <h3 className="text-xl font-semibold mb-2">{t('myTournaments.noActive')}</h3>
                <p className="text-muted-foreground mb-4">
                  {t('myTournaments.noActiveDesc')}
                </p>
                <Button
                  onClick={() => navigate('/tournaments')}
                  className="btn-mystic text-white"
          ...
          <TabsContent value="completed">
            {completedTournaments.length === 0 ? (
              <Card className="card-mystic text-center py-12">
                <Trophy className="w-16 h-16 mx-auto text-primary/50 mb-4" />
                <h3 className="text-xl font-semibold mb-2">{t('myTournaments.noCompleted')}</h3>
                <p className="text-muted-foreground">
                  {t('myTournaments.noCompletedDesc')}
                </p>
              </Card>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
