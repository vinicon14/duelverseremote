  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await (supabase as any).rpc("create_normal_tournament", {
        ...
        p_requires_decklist: requiresDecklist,
      });
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between rounded-lg border p-4">
                <div>
                  <Label htmlFor="requiresDecklist" className="font-medium">Exigir Decklist</Label>
                  <p className="text-xs text-muted-foreground mt-1">
                    Participantes precisarão enviar uma imagem da decklist ao se inscrever
                  </p>
                </div>
                <Switch
                  id="requiresDecklist"
