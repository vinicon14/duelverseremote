...
// Extend Window interface for ad blocking
declare global {
  interface Window {
    _monetagBlocked?: boolean;
    _originalOpen?: typeof window.open;
    _monetagObserver?: MutationObserver;
    _monetagInterval?: number;
    _duelRoomAdCleanup?: number;
  }
}

// Complete ad blocking function for DuelRoom
const enableCompleteAdBlocking = () => {
  console.log('DuelRoom: Ativando bloqueio COMPLETO de anúncios');
  
  // Block ALL window.open calls
  const originalOpen = window.open;
  window.open = function(...args: Parameters<typeof window.open>): Window | null {
    console.log('DuelRoom: BLOQUEADO - tentativa de abrir nova guia/popup');
    return null;
  };
  ...
  const cleanup = () => {
    ...
    document.querySelectorAll('script').forEach((script) => {
      ...
      if (
        ...
        script.remove();
        console.log('DuelRoom: Removido script de anúncio');
      }
    });

    // Remove ALL ad-related iframes
    document.querySelectorAll('iframe').forEach((iframe) => {
      ...
      if (
        ...
        iframe.remove();
        console.log('DuelRoom: Removido iframe de anúncio');
      }
    });

    // Remove ALL popup/overlay/div ad elements
    document.querySelectorAll('*').forEach((el) => {
      ...
      const isAdElement = 
        (style.includes('fixed') && style.includes('z-index') && parseInt(style.match(/z-index:\s*(\d+)/)?.[1] || '0') > 1000) ||
        id.includes('quge5') || 
        id.includes('monetag') ||
        id.includes('popup') ||
        ...
        className.includes('ads') ||
        (el.getAttribute && (el.getAttribute('data-quge5') || el.getAttribute('data-monetag') || el.getAttribute('data-ads')));
      
      if (isAdElement) {
        el.remove();
        console.log('DuelRoom: Removido elemento de anúncio:', id || className);
      }
    });
  };
  ...
  // Aggressive MutationObserver for DuelRoom
        ...
        if (node.nodeType === 1) {
          ...
          if (el.tagName === 'SCRIPT') {
            ...
            if (
              ...
              el.remove();
              console.log('DuelRoom: Bloqueado script injetado');
            }
          }
          ...
          if (el.tagName === 'IFRAME') {
            ...
            if (
              ...
              el.remove();
              console.log('DuelRoom: Bloqueado iframe injetado');
            }
          }
          ...
          if (isAdElement) {
            el.remove();
            console.log('DuelRoom: Bloqueado elemento de anúncio injetado');
          }
        }
      });
    });
  ...
  (window as any)._monetagObserver = observer;

  // Frequent cleanup for DuelRoom (every 2 seconds)
  const cleanupInterval = setInterval(cleanup, 2000);
  (window as any)._duelRoomAdCleanup = cleanupInterval;

  return () => {
    console.log('DuelRoom: Limpando bloqueador de anúncios');
    observer.disconnect();
...
const DuelRoom = () => {
  ...
  // Enable complete ad blocking for DuelRoom
  useEffect(() => {
    const cleanup = enableCompleteAdBlocking();
    return cleanup;
  }, []);
