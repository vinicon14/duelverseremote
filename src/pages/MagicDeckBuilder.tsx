...
export default function MagicDeckBuilder() {
  ...
  const handleSave = async () => {
    ...
    const deckData = {
      user_id: user.id,
      name: deckName,
      main_deck: mainDeck.map(c => ({ id: c.card.id, name: c.card.name, quantity: c.quantity, image: getCardImage(c.card) })),
      side_deck: sideboard.map(c => ({ id: c.card.id, name: c.card.name, quantity: c.quantity, image: getCardImage(c.card) })),
      extra_deck: [],
      tokens_deck: [],
      tcg_type: 'magic'
    };
