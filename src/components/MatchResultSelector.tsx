  ...
  const reportResult = async (winnerId: string) => {
    try {
      ...
      if (!error && data?.success) {
        toast({
          title: t('matchResult.reportedTitle'),
          description: data.next_round_generated 
            ? t('matchResult.nextRound')
            : data.message,
        });
        setIsOpen(false);
        onResultReported();
      ...
      toast({
        title: t('matchResult.reportedTitle'),
        description: t('matchResult.updatedSuccess'),
      });
      ...
      toast({
        title: t('matchResult.reportError'),
      ...
      <DialogContent className="sm:max-w-md card-mystic">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('matchResult.reportTitle')}
          </DialogTitle>
          <DialogDescription>
            {t('matchResult.reportDesc')}
          </DialogDescription>
        </DialogHeader>
