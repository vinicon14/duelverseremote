 ...
 * Each injected script is marked as an authorized Monetag element so the
 * NoMonetagAds/ProAdCleaner cleanup components don't accidentally remove them.
 */
export const ConditionalMonetagLoader = (): null => {
  ...
  useEffect(() => {
    ...
    markAuthorizedMonetagElement(notificationScript1);
    ...
    markAuthorizedMonetagElement(notificationScript2);
...
const applyPopupBlocking = () => {
  ...
  window.open = function (...args: Parameters<typeof window.open>): Window | null {
    ...
    return window._originalOpen?.(...args);
  };
};
