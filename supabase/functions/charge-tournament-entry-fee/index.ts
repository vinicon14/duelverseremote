  ...
  try {
    ...
    if (tournament.created_by === user.id) {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Você não pode se inscrever no seu próprio torneio",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (tournament.status !== "upcoming") {
      return new Response(
        JSON.stringify({
          success: false,
          message: "Este torneio não está aceitando inscrições",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (
      ...
      profile.duelcoins_balance < tournament.entry_fee
    ) {
      return new Response(
        JSON.stringify({
          success: false,
          message:
            `Saldo insuficiente. Você precisa de ${tournament.entry_fee} DuelCoins`,
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.error("Error in charge-tournament-entry-fee:", error);
    return new Response(
      JSON.stringify({
        success: false,
