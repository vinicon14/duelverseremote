-- Fix for the matchmaking ambiguity error
...
BEGIN
  ...
  IF v_opponent.user_id IS NOT NULL THEN
    ...
    -- FIX: Use table alias or qualified name to avoid ambiguity with RETURNS TABLE column "status"
    UPDATE public.matchmaking_queue AS mq_upd
    SET status = 'matched', duel_id = v_new_duel
    WHERE mq_upd.user_id IN (p_user_id, v_opponent.user_id)
      AND mq_upd.status = 'waiting';
